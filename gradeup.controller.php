...
class gradeupController extends gradeup {
	...
	function triggerDoLoginAfter(&$obj){
		...
		//등업기능 사용시 작동
		if($module_config->gradeup_use != 'yes') return;
				...
				if(!in_array($val->title, $member_groups)){
					...
					if($priority <= $group['gradeup_priority']){
						...
						if($group['gradeup_doc'] || $group['gradeup_com'] || $group['gradeup_lv']){
							//조건 없는항목은 통과
							if(!$group['gradeup_doc']) $condition_doc = true;
							if(!$group['gradeup_com']) $condition_com = true;
							if(!$group['gradeup_lv']) $condition_lv = true;

							//게시글조건 확인
							if($group['gradeup_doc']){
								if($group['gradeup_date']) $date = $group['gradeup_date'];
	...
	function procGradeupConfirmGroup(){
		...
		//회원정보 없을시 리턴
		if(!$logged_info) $this->setMessage('로그인후 이용가능합니다.');
		if(!$logged_info) return $this->setRedirectUrl(getNotEncodedUrl('','module','gradeup','act','dispGradeupConfirmGroup'));
